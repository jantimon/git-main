...
test("custom-branch: git-main uses custom branch when specified as argument", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    assert.strictEqual(
      developContent,
      "develop content",
      "Should have develop branch content",
    );
  });
});
...
test("custom-branch: git-main checks out remote branch when it exists on remote but not locally", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    const remoteBranches = testAPI
      ...
      .trim();
    assert(
      remoteBranches.includes("origin/feature/remote-only"),
    ...
    assert.strictEqual(
      currentBranch,
      "feature/remote-only",
      "Should be on feature/remote-only branch",
    );

    // Verify the branch is tracking the remote
    const trackingInfo = testAPI.exec("git branch -vv").trim();
    assert(
      trackingInfo.includes("[origin/feature/remote-only]"),
      "Branch should be tracking remote",
    );
    ...
    assert.strictEqual(
      remoteContent,
      "remote content",
      "Should have remote branch content",
    );
  });
});
...
test("custom-branch: git-main creates new branch when it does not exist and user confirms", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    assert.strictEqual(
      currentBranch,
      "feature/new-branch",
      "Should be on feature/new-branch branch",
    );
    ...
    assert.strictEqual(
      mainCommit,
      newBranchCommit,
      "New branch should be created from latest main",
    );
  });
});
...
test("custom-branch: git-main creates new branch with dirty working directory", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    assert.strictEqual(
      ...
      "Should be on feature/dirty-branch branch",
    );
...
test("custom-branch: git-main exits when user declines to create new branch", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    assert.strictEqual(
      result.code,
      1,
      "Should exit with code 1 when user declines",
    );
...
test("custom-branch: git-main creates new branch from non-main branch", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    assert.strictEqual(
      parentCommit,
      childCommit,
      "Child branch should be created from parent branch",
    );
    ...
    assert.strictEqual(
      ...
      "Should have parent branch content",
    );
    ...
    assert.notStrictEqual(
      childCommit,
      mainCommit,
      "Child branch should not be created from main",
    );
  });
});
