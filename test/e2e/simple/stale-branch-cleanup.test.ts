...
test("stale-branch-cleanup: git-main detects and cleans up branches with deleted remotes", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    assert(
      branchList.includes("feature/test-branch"),
      "Branch should exist with remote tracking",
    );
    assert(
      branchList.includes("[origin/feature/test-branch]"),
      "Branch should have remote tracking info",
    );
    ...
    assert(
      branchListAfterPrune.includes("gone]"),
      "Branch should show as gone after remote prune",
    );
    ...
    assert(
      output.includes("Deleting branch feature/test-branch"),
      "Should mention deleting the branch",
    );
    assert(
      output.includes("Deleted 1 branch"),
      "Should confirm 1 branch was deleted",
    );
...
test("stale-branch-cleanup: git-main handles case with no branches with deleted remotes", async () => {
  await setupTemporaryTestEnvironment(__dirname, async (testAPI) => {
    ...
    const branchList = testAPI.exec("git branch -vv");
    assert(
      branchList.includes("feature/active-branch"),
      "Branch should exist with remote tracking",
    );
    assert(
      branchList.includes("[origin/feature/active-branch]"),
      "Branch should have remote tracking info",
    );
    assert(
      !branchList.includes("gone]"),
      "Branch should NOT show as gone since remote still exists",
    );
    ...
    assert(
      output.includes("No branches with deleted remotes found"),
      "Should mention no branches found",
    );
    ...
    assert(
      finalBranchList.includes("feature/active-branch"),
