...
test.describe("package-lock-change-simple", () => {
  test(
    "should run npm ci if feature branch lockfile differs from main branch lockfile",
    { timeout: 20000 },
    async () => {
      await setupTemporaryTestEnvironment(
        __dirname,
        async (testAPI: TestAPI) => {
          ...
          const initialLockfileFromFixtures = fs.readFileSync(
            path.join(__dirname, "fixtures", "initial", "package-lock.json"),
            "utf8",
          );
          assert.deepStrictEqual(
            JSON.parse(lockfileAfterGitMain),
            JSON.parse(initialLockfileFromFixtures),
            "Lockfile on disk should be main's (initial) version after script execution",
          );

          assert.ok(
            gitMainOutput.includes("Installing dependencies with npm"),
            'Output should include spinner message "Installing dependencies with npm"',
          );
          assert.strictEqual(
            gitMainErrorOutput,
            "",
            "No errors should occur during git-main.js execution",
          );

          const branches = exec("git branch");
          assert.ok(
            branches.includes("* main"),
            "Should be on main branch after script execution",
          );
          assert.ok(
