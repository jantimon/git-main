...
async function findStaleBranches(mainBranch) {
  try {
    ...
    for (const line of branches) {
      const match = line.match(/^\s*(\*?\s*)([^\s]+)\s+[a-f0-9]+(?:\s+\[([^\]]+)\])?\s*(.*)/);
      if (!match) continue;
      ...
      if (trackingInfo.includes(': gone')) {
        ...
        const unpushedCommits = cherryOutput.stdout.split('\n').filter(line => line.startsWith('+')).length;
    ...
    log.error(`Error finding stale branches: ${e instanceof Error ? e.message : e}`);
    return [];
  }
}
...
function displayStaleBranches(groupedBranches) {
  ...
  const hasMultipleCategories = Object.values(groupedBranches).filter(arr => arr.length > 0).length > 1;
  
  for (const [category, branches] of Object.entries(groupedBranches)) {
    if (branches.length === 0) continue;
    
    if (hasMultipleCategories) {
      const title = categoryTitles[/** @type {keyof typeof categoryTitles} */(category)];
      console.log(chalk.bold(title) + ':');
    }
    
    for (const branch of branches) {
      const unpushedInfo = branch.unpushedCommits > 0 
        ? `, has ${branch.unpushedCommits} unpushed commit${branch.unpushedCommits > 1 ? 's' : ''}`
        : '';
      
      console.log(`→ ${chalk.yellow(branch.name)} (${formatAge(branch.age)}) - remote deleted${unpushedInfo}`);
...
async function main() {
  ...
  if (staleBranches.length === 0) {
    ...
    console.log(`Found ${staleBranches.length} branch${staleBranches.length > 1 ? 'es' : ''} to clean up:`);
    ...
    if (allBranches.length > 0) {
      ...
      if (allBranches.length > 1) {
        console.log("");
        console.log(`Delete all ${allBranches.length} stale branch${allBranches.length > 1 ? 'es' : ''}? [Enter to pick individually]`);
        
        while (choice !== "y" && choice !== "n" && choice !== "") {
          choice = await question(`Delete all? [${chalk.green('y')}/${chalk.red('n')}/${chalk.yellow('Enter')}]: `);
        }
      }
      ...
      if (choice === "y") {
        ...
        log.success(`Deleted ${allBranches.length} stale branch${allBranches.length > 1 ? 'es' : ''}`);
      } else if (choice === "") {
        // Individual review mode
        console.log("");
        for (const branch of allBranches) {
          const unpushedInfo = branch.unpushedCommits > 0 
            ? ` (${branch.unpushedCommits} unpushed commit${branch.unpushedCommits > 1 ? 's' : ''})`
            : '';
          
          const shouldDelete = await confirmAction(
            `Delete ${chalk.bold(branch.name)} (${formatAge(branch.age)})${unpushedInfo}?`
          );
